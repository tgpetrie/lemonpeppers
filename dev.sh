...
case "${1:-help}" in
 ...
 then
    ...
    "health")
        ...
        BACKEND_PORTS_IN_USE=0
        for port in 5001 5002 5003 5004 5005; do
            if lsof -i :$port >/dev/null 2>&1; then
                BACKEND_PORTS_IN_USE=$((BACKEND_PORTS_IN_USE + 1))
            fi
        done
        
        if [ $BACKEND_PORTS_IN_USE -eq 0 ]; then
            print_success "Backend ports: Available"
        else
            print_warning "Backend: $BACKEND_PORTS_IN_USE of 5 common ports in use"
        fi
        
        if ! lsof -i :5173 >/dev/null 2>&1; then
            print_success "Frontend port 5173: Available"
        else
            print_warning "Frontend port 5173: In use (will auto-select alternative)"
        fi
        ;;
    ...
    "status")
        ...
        # Check for backend on common ports
        BACKEND_FOUND=false
        for port in 5001 5002 5003 5004 5005 5006 5007 5008 5009 5010; do
            BACKEND_PID=$(lsof -t -i:$port 2>/dev/null || true)
            if [ -n "$BACKEND_PID" ]; then
                # Check if process exists and contains python app.py
                if ps -p $BACKEND_PID >/dev/null 2>&1; then
                    PROCESS_CMD=$(ps -p $BACKEND_PID -o args= 2>/dev/null || echo "")
                    if echo "$PROCESS_CMD" | grep -q "app.py"; then
                        print_success "Backend server is running on port $port (PID: $BACKEND_PID)"
                        BACKEND_FOUND=true
                        break
                    fi
                fi
        ...
        for port in 5173 5174 5175 5176 5177; do
            FRONTEND_PID=$(lsof -t -i:$port 2>/dev/null || true)
            if [ -n "$FRONTEND_PID" ]; then
                # Check if process exists and contains vite or npm dev
                if ps -p $FRONTEND_PID >/dev/null 2>&1; then
                    PROCESS_CMD=$(ps -p $FRONTEND_PID -o args= 2>/dev/null || echo "")
                    if echo "$PROCESS_CMD" | grep -q "vite\|npm.*dev"; then
                        print_success "Frontend server is running on port $port (PID: $FRONTEND_PID)"
                        FRONTEND_FOUND=true
                        break
                    fi
                fi
    ...
    "stop")
        ...
        # Stop backend servers on common ports
            ...
            if [ -n "$BACKEND_PID" ]; then
                ...
                if ps -p $BACKEND_PID >/dev/null 2>&1; then
                    ...
                    if echo "$PROCESS_CMD" | grep -q "app.py"; then
                        ...
                        print_success "Backend server stopped (was on port $port)"
        ...
        if [ "$BACKEND_STOPPED" = false ]; then
            print_warning "No backend server found to stop."
        fi

        # Stop frontend servers on common ports
            ...
            if [ -n "$FRONTEND_PID" ]; then
                ...
                if ps -p $FRONTEND_PID >/dev/null 2>&1; then
                    ...
                    if echo "$PROCESS_CMD" | grep -q "vite\|npm.*dev"; then
                        ...
                        print_success "Frontend server stopped (was on port $port)"
        ...
        if [ "$FRONTEND_STOPPED" = false ]; then
            print_warning "No frontend server found to stop."
        fi
        ;;
